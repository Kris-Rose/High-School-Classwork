...
public class WordCloudGUI {
    ...
    {
        WordCloud w = new WordCloud("dream.txt"); //replace with your file's location, mine is in a folder called data
    ...
    protected void initUI(ArrayList<Word> wds) {
        ...
        Font font = new Font(fonts[0], Font.PLAIN, 0);
        for (int i = 0; i < this.cloud.size(); i++) {
            JLabel label = new JLabel("  " + cloud.get(i).getWord() + "  ");
            int r = (int)(Math.random() * 256);
            int g = (int)(Math.random() * 256);
            int b = (int)(Math.random() * 256);
            label.setForeground(new Color(r, g, b));
            label.setOpaque(false);
            label.setFont(new Font(font.getFontName(), Font.PLAIN,
